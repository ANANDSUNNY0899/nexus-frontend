...
export default function Dashboard() {
  ...
  // 1. Prepare the new history segment
  const newUserMsg = { role: "user", content: message };
  const updatedHistory = [...chatHistory, newUserMsg];
  
  // 2. Reset UI for the new stream
  setChatLoading(true)
  setChatResponse("")
  setReasoning("")
  setQuotaExceeded(false)
  ...
  // Update local history immediately so the UI feels fast
  setChatHistory(updatedHistory);
  const currentPrompt = message; // Store to clear input
  setMessage(""); 
  ...
  // (Include your BYOK Header logic here as before...)
  ...
  try {
    const res = await fetch(`${backendUrl}/api/chat/stream`, {
      ...
      body: JSON.stringify({ 
        model: model, 
        messages: updatedHistory // <--- SENDING FULL ARRAY TO GO
      }),
    })
    ...
    const reader = res.body?.getReader()
    const decoder = new TextDecoder()
    let fullAiResponse = ""; // To capture the final text
      ...
      for (const line of lines) {
        if (line.startsWith("data: ") && !line.includes("[DONE]")) {
          try {
            ...
            // Capture Telemetry & Reasoning as before...
    ...
    // 3. CRITICAL: Save the AI's response to history so it remembers next time
    setChatHistory(prev => [...prev, { role: "assistant", content: fullAiResponse }]);
